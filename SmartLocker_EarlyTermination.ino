...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) { 
    ...
    // Timeout after 30 seconds (60 * 500ms)
    if (w > 60) { 
        tft.fillScreen(BG_COLOR);
        centerText("WiFi Failed", 120, 3, ERROR_COLOR);
        centerText("Restarting...", 160, 2, ERROR_COLOR);
        delay(3000);
        ESP.restart();
    }
  }
  ...
  tft.fillScreen(BG_COLOR);
  centerText("Syncing Time...", 150, 2, TEXT_COLOR);
